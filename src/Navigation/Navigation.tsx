...
export const Navigation = ({state, setMenuIsOpen, menuIsOpen}: any) => {
    ...
    const menuHandler = () => {
        setMenuIsOpen((prevState: number) => !prevState)
    }
        ...
        <div className='navigation'>
            <div  className={menuIsOpen && width < 1023 ? 'navigation__wrapper__hidden' : 'navigation__wrapper'}>
                <div className={width < 1023 ? 'navigation__links' : 'navigation__links navigation__links_desktop'}>
                    ...
                    {width < 1023 ? <Link className={!menuIsOpen && state ? 'navigation__link' : 'navigation__link navigation__link_hidden' } to='/dictionary' onClick={() => setMenuIsOpen(true)}>Dictionary</Link> : <Link className={ state ? 'navigation__link' : 'navigation__link navigation__link_hidden' } to='/dictionary' onClick={() => setMenuIsOpen(true)}>Dictionary</Link>}
